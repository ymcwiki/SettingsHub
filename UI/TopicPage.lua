...
local function buildMuteSection(page)
	...
	addBtn:SetSize(Style.MuteAddWidth, Style.MuteAddHeight)
	...
	function page:RefreshMuteList()
		...
		for i, fileID in ipairs(ns.db.profile.mutesound) do
			...
			if not row then
				...
				row.temp = CreateFrame("Button", nil, row, "UIPanelButtonTemplate")
				row.temp:SetSize(Style.MuteTempWidth, Style.MuteTempHeight)
				row.temp:SetPoint("LEFT", 160, 0)
				row.temp:SetText(L["Unmute now"])
				row.del = CreateFrame("Button", nil, row, "UIPanelButtonTemplate")
				row.del:SetSize(Style.MuteRemoveWidth, Style.MuteRemoveHeight)
				row.del:SetPoint("LEFT", row.temp, "RIGHT", 6, 0)
				row.del:SetText(L["Remove"])
				self.muteRows[i] = row
			end
			row:ClearAllPoints()
...
local function buildTopicPage(topicKey)
	return function(parent)
		...
		local curatedHeader = curated:CreateFontString(nil, "OVERLAY", "GameFontNormal")
		curatedHeader:SetPoint("TOPLEFT", 4, -4)
		curatedHeader:SetText(L["Curated picks"])

		page.reloadBtn = CreateFrame("Button", nil, curated, "UIPanelButtonTemplate")
		page.reloadBtn:SetSize(Style.ReloadButtonWidth, Style.ReloadButtonHeight)
