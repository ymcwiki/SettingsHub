...
- **词典层百科**:1,015 条 CVar 白话解释悬停即见,三级可信度标注(人工核验/社区词典/未公开),全量解释覆盖率 70%,中英文描述全文可搜
- **全量 CVar 浏览器**:运行时枚举 1,600+ 条,名称/描述/中英俗名全文搜索,`tag:favorite` `tag:modified` `tag:new` `tag:secure` `tag:hidden` 过滤词,类别侧栏,右键收藏常用项,行内编辑,非默认值高亮,默认值独立列
