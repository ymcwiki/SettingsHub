...
- [ ] 打开 `/sh` 默认落在发现页;建议区按当前值显示(盔甲摩擦音开着时出现对应建议),一键处理后建议消失,「不再提示」后重登也不再出现
- [ ] 八条意图引导逐条过:内联控件可改可撤销,与主题页行为一致;带包的引导「打开对应推荐包」跳转正确
- [ ] 补丁新增区:12.0 项可见可改,12.1 项在 12.0.7 上显示「不可用」计数

**试穿模式(推荐包页)**

- [ ] 「试穿 10 分钟」:值生效但 tag:modified 之外期望态不动(日志页来源显示「试穿」);顶部状态条出现剩余时间
- [ ] 到期自动还原;试穿中登出再登录,值已还原且有提示
- [ ] 「立即还原」「转为常驻」两个按钮行为正确;转常驻后重登值被重放守住
- [ ] 试穿中再点另一包的试穿:提示先处理当前试穿

**手感**

- [ ] 数值滑块:拖动数字实时变、松手才写入(日志页只记一条);旁边输入框精确改值一致
- [ ] 枚举下拉:点开是单选菜单,当前值有选中标记;选择立即生效
- [ ] 小地图按钮:点击开关主窗口,拖动沿小地图边缘移动且重登记住位置
